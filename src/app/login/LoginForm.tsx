...
export default function LoginForm() {
  ...
  useEffect(() => {
    ...
    setNextPath(next && next.startsWith("/") ? next : "/dashboard");
  }, []);
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen px-4 ${darkMode ? "bg-gray-900 text-white" : "bg-blue-50 text-gray-900"}`}>
      ...
      <div className={`p-8 rounded-xl shadow-lg w-full max-w-md space-y-6 border ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
        ...
        <button onClick={() => signIn("google", { callbackUrl: nextPath })} className="flex items-center justify-center w-full border border-gray-300 dark:border-gray-600 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
        ...
        {nextPath.startsWith("/admin") && (
          <button
            type="button"
            onClick={() => router.push("/signup?admin=1&next=%2Fadmin")}
            className="flex items-center justify-center w-full border border-indigo-300 text-indigo-700 dark:text-indigo-300 dark:border-indigo-500 py-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-gray-700 transition"
          >
